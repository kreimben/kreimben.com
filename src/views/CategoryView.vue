<template>
    <v-container>
        ...
        >Category: {{ this.category.name }}</v-card>

        <v-row>
            <v-col v-for="(post, index) in this.posts" :key="index" cols="12" sm="4" md="4" lg="4" xl="4">
                <v-card class="py-16 ma-4" @click="$router.push({ path: `/posts/${post.id}` })">
